...
const Navigation = ({isMenuOpen, onMenuToggle}: Props) => {
  ...
  const navClasses = isMenuOpen ? "pre-md:text-black" : "pre-md:hidden"
  ...
  return (
    <nav className={`md:justify-self-start
        ...
        pre-md:flex pre-md:justify-center pre-md:items-center
        ${navClasses}`}
      ...
      <ul className={`list-none flex flex-col md:flex-row`}>
        {categories.map((category) => (
          <li key={category.title} className="px-2 pre-md:mb-8">
            <NavLink onClick={onMenuToggle} className={navLinkActiveClasses} to={`shop/${category.title.toLowerCase()}`}>
              {category.title}
            </NavLink>
          </li>
        ))}
